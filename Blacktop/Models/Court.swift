...
enum DataSource: String, Codable, CaseIterable {
    ...
    var displayName: String {
        ...
        case .blacktopManual: "Blacktop manual check"
    ...
    init(from decoder: Decoder) throws {
        ...
        let legacyManualSource = ["hoop", "Life", "Manual"].joined()
        if value == legacyManualSource {
            self = .blacktopManual
            return
        }
        guard let source = DataSource(rawValue: value) else {
            throw DecodingError.dataCorruptedError(in: container, debugDescription: "Unknown data source: \(value)")
        }
        self = source
    }
}
...
enum DataConfidence: String, Codable, CaseIterable {
    ...
    var displayName: String {
        switch self {
        case .imported: "Public data"
        case .candidateImported: "Candidate"
        case .needsCheck: "Needs details"
        case .userSuggested: "User suggested"
    ...
    var isImportedLike: Bool {
        self == .imported || self == .candidateImported
    }
}
