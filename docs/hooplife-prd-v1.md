# HoopLife iOS App PRD v1

## 1. Product Summary

HoopLife is a lightweight basketball court discovery app focused on factual court information, starting with Sheffield and expanding to major UK cities.
...
HoopLife helps basketball players find real, usable courts before leaving home.
...
1. Facts over opinions
   HoopLife should avoid vague ratings such as stars or scores. A court is useful because it has lights, is free, is open at night, or has a playable surface.
...
HoopLife focuses on these missing practical facts.
...
OpenStreetMap data is licensed under ODbL. HoopLife can use and adapt it, but must credit OpenStreetMap contributors and respect ODbL share-alike obligations when distributing adapted OSM-derived databases.
...
- Bulk importing Google Places results into HoopLife
- Treating Google as the source of the long-term HoopLife database
...
HoopLife should separate data into three conceptual layers:

1. Source layer
   Imported facts from OSM, Active Places, or other allowed sources.

2. HoopLife enrichment layer
   Fields manually checked or added by HoopLife, such as "good for solo shooting" or "gate locked after sunset."

3. User contribution layer
   User-submitted edits, marked as pending or confirmed.

This matters because imported open data may require attribution or license compliance. HoopLife's own verified/enriched fields can become the product's proprietary value, but source provenance should remain stored.
...
Data came from an external open dataset and has not been checked by HoopLife.
...
HoopLife or a trusted contributor has manually checked the key facts.
...
HoopLife should feel practical, young, and urban, but not noisy.
...
3. Normalize fields into HoopLife schema.
...
- Keep HoopLife enrichment fields separate from imported source fields.
...
4. Build a seed CSV with HoopLife fields.
